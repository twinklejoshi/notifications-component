...
describe("Notifications Component", () => {
  ...
  it("should mark a notification as read when clicked", async () => {
    ...
    const firstNotification =
      screen.getByText("Notification 1").parentElement?.parentElement!;
  ...
  it("should open a new tab with the correct URL when a notification is clicked", () => {
    ...
    const notification = mockNotifications[0]; //clicking first notification

    const firstNotification = screen.getByText("Notification 1");
    fireEvent.click(firstNotification);

    // Assertions
    expect(mockOpen.mock.results[0].value.notification).toBe(notification);
